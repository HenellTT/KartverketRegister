...
class UserManagerPage {
    constructor(userId, roleSetUser, roleSetAdmin, roleSetEmployee, deleteButton, msgInput, msgSubmit, csrfToken) {
        ...
        this._msgSubmit.addEventListener('click', () => this.SendNotification());
    ...
    async SendNotification() {
        let msg = this._msgInput.value;
        await fetch(`/Superadmin/SendNotification`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': this._csrfToken
            },
            body: JSON.stringify({ Id: this._userId, Email: msg}) // Email is actually the msg im just tooooooo lazy to add a new var to the model
        });
        window.location.href = '/';
